...
from thumbnail import generate_thumbnail, make_thumbnail_buffer
...
class ServiceRunner(dl.BaseServiceRunner):
    ...
    def run(self, item: dl.Item, context=None, progress: dl.Progress | None = None) -> None:
        ...
        try:
            ...
            if do_thumbnail:
                generate_thumbnail(item, self.png_image, self.default_thumb_size)
    ...
    def _probe_tiff(self, item: dl.Item) -> None:
        ...
        exifdata = read_exif_tags(item, self.pil_image)
        self.is_multibit = self._get_bits_per_sample(item, exifdata)
        self.exif_data = extract_exif(item, exifdata)
